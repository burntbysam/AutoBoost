...
- **Dialog-aware recovery** (0.7.19–0.7.21): when a part fails mid-cycle, closing
  its unsaved Design/Cut window makes Boost pop a "Save changes?" prompt. The old
  ...
  prompts and dismisses them by clicking only safe buttons — Don't Save / No /
  Cancel / OK, never Save/Yes/Exit — closes any leftover window, and confirms a
  clean Home. The close is hardened (0.7.20): foreground the window over RDP
  (Win32 SetForegroundWindow), several Esc to cancel any active tool that would
  swallow the close key, then `3`, then Alt+F4 as a fallback — logging which
  path worked. Dialog detection is hardened too (0.7.21, after the 0.7.20 run
  showed the prompt going undetected): prompts are found via each Boost
  window's *enabled popup* (the OS's own record of the modal a window is
  waiting on — immune to process/title surprises) plus a scan across **all**
  Boost processes, since Home (WPF) and Design (WinForms) need not share one;
  the prompt is dismissed **before** judging whether the close worked, and a
  stuck close logs the foreground window, whether Design is disabled (= a
  modal is up), and a census of Boost's windows. Still scoped to Boost's own
